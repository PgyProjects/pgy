...
<html>
	...
	<head>
		...
		<link rel="stylesheet" type="text/css" href="/assets/houtai2/css/cuishou_style.css" />
		...
		<div class="alertInp">
			...
			<div class="alertInp_main">
				...
				<div class="alertRight col-lg-4">
					...
					<div class="chioce_main" id="cuishou" style="display: none;">
						<!--<div class="recordTop">
							<div  ><span>状态:</span><span>2</span></div>
					...
					<div class="right_footer">
						...
						<div class="pass_btn btn btn-success" id="hasHk">已还款</div>
						<div class="nopass_btn btn btn-warning" id="yanqi">延期</div>
						<div class="wait_btn btn btn-danger" id="yuqi">逾期中</div>
					</div>
					<div class="showAlert">
						<div class="input-group mTop">
							<span class="input-group-addon">延期金额:</span>
							<input type="number" class="form-control" id="yqMoney">
						</div>
						<div class="input-group mTop">
							<span class="input-group-addon">延期开始日期:</span>
							<input type="text" class="form-control" id="yqTime">
						</div>
						<div class="input-group mTop">
							<span class="input-group-addon">延期天数:</span>
							<input type="number" class="form-control" id="yqTotime">
						</div>
						<div class="input-group mTop">
							<span class="input-group-addon">延期至:</span>
							<input type="text" class="form-control" id="yqGotime" readOnly="true">
						</div>
						<div class="input-group mTop">
							<span class="input-group-addon">延期费:</span>
							<input type="number" class="form-control" id="yqFee">
						</div>
						<button type="button" class="btn btn-primary mMiddle" id="postYq">提交</button>
					</div>
				</div>
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		$('.choiceTime li').click(function() {
			...
			if($(this).text() == '当日还款客户') {
				jsonType(1, 1);
			} else if($(this).text() == '宽限日客户') {
				jsonType(1, 2);
			} else if($(this).text() == '逾期(2-7)') {
				jsonType(1, 3);
			} else if($(this).text() == '逾期(8-15)') {
				jsonType(1, 4);
			} else if($(this).text() == '逾期(16-30)') {
				jsonType(1, 5);
			}
		})
		jsonType(1, 0);

		function jsonType(page, oNum) {
			var data_jk = '';
			if(oNum == 0) {
				data_jk = '';
			} else if(oNum == 1) {
			...
			$.ajax({ //获取数据
				...
				success: function(data) {
					...
					for(var i = 0; i < data.data.length; i++) {
						...
						str += '<tr class="showAlerts" id="' + data.data[i].jk_id + '">';
		...
		function showDo(id, a) {
			$.ajax({ //获取数据
				...
				success: function(data) {
					...
					str += '<li id="weixin_tx" class="weixin_tx">';
					...
					document.getElementById('shPeople').innerHTML = '催收员:' + data.manager_name;
					/*放款记录*/
					var str2 = '';
					var listNum = data.jiekuan_list.length;
					if(listNum != '0') {
						str2 += '<div class="recordTop">';
						str2 += '<div  ><span>累计笔数:</span><span>' + listNum + '</span></div>';
						str2 += '<div  ><span>当前金额:</span><span class="color-red">' + data.jiekuan_list[listNum - 1].amount + '</span></div>';
						var allMoney = 0;
						for(var k = 0; k < listNum; k++) {
							allMoney += parseInt(data.jiekuan_list[k].amount);
						}
						...
						for(var j = 0; j < listNum; j++) {
							...
							if(data.jiekuan_list[j].status == '0') {
								str2 += '<span class="titleFont">未还款</span>';
							} else if(data.jiekuan_list[j].status == '1') {
								oImg = 'assets/houtai2/img/hk.PNG';
								str2 += '<span class="titleFont">已还款</span>';
								str2 += '<img src="<?php echo asset("' + oImg + '") ?>" />';
							} else if(data.jiekuan_list[j].status == '2') {
								oImg = 'assets/houtai2/img/yq.PNG';
								str2 += '<span class="titleFont">有逾期</span>';
								str2 += '<img src="<?php echo asset("' + oImg + '") ?>" />';
							}
							str2 += '</div>';
							str2 += '<div class="topMoney">' + data.jiekuan_list[j].amount + '</div>';
							str2 += '</div>';
							str2 += '<div class="listMain">';
							str2 += '<div class="line_right">';
							str2 += '<span>借款日期:&nbsp;' + data.jiekuan_list[j].jkdate + '</span><br />';
							var allDay = Date.parse(data.jiekuan_list[j].hkdate) - Date.parse(data.jiekuan_list[j].jkdate);

							str2 += '<span>借款天数:&nbsp;' + parseInt(allDay / 3600 / 24 / 1000) + '&nbsp;天</span>';
							str2 += '</div>';
							str2 += '<div   style="float: right;">';

							str2 += '<span>计划还款日:&nbsp;' + data.jiekuan_list[j].hkdate + '</span><br />';
							str2 += '<span>实际还款日:&nbsp;' + data.jiekuan_list[j].paydate + '</span>';
							str2 += '</div>';
							str2 += '</div>';

							for(var n = 0; n < data.jiekuan_list[j].cs_list.length; n++) {
								str2 += '<div class="listMain">';
								str2 += '<div class="line_right">';
								str2 += '<span>延期金额:&nbsp;' + data.jiekuan_list[j].cs_list[n].amount + '</span><br />';
								str2 += '<span>延期费用:&nbsp;' + data.jiekuan_list[j].cs_list[n].fee + '</span><br />';
								str2 += '<span>延期天数:&nbsp;' + data.jiekuan_list[j].cs_list[n].days + '&nbsp;天</span>';
								str2 += '</div>';
								str2 += '<div   style="float:right;">';
								str2 += '<span>延期开始日:&nbsp;' + data.jiekuan_list[j].cs_list[n].begin_date + '</span><br />';

								str2 += '<span>延期结束日:&nbsp;' + data.jiekuan_list[j].cs_list[n].end_date + '</span>';
								//								str2 += '<span>实际还款日:&nbsp;' + data.jiekuan_list[j].paydate + '</span>';
								str2 += '</div>';
								str2 += '</div>';
								...
								str2 += '<li>延期备注:</li>';
								str2 += '<li class="bzNr">';
								str2 += '<span>' + data.jiekuan_list[j].cs_list[n].note + '</span>';
								str2 += '</li>';
								str2 += '</ul>';
								str2 += '</div>';
							}
							...
							str2 += '<li>备注:</li>';
							str2 += '<li class="bzNr">';
							str2 += '<span>' + data.jiekuan_list[j].note + '</span>';
							str2 += '</li>';
							str2 += '</ul>';
							str2 += '</div>';
							str2 += '</div>';
					...
					/*催收记录*/
					var str3 = '';
					for(var n = 0; n < data.cuishou_list.length; n++) {
						str3 += '<div class="recordTop" style="margin-bottom:10px;">';
						var oType;
						if(data.cuishou_list[n].status == '0') {
							oType = '逾期 ';
						} else if(data.cuishou_list[n].status == '1') {
							oType = '承诺还款 ';
						} else if(data.cuishou_list[n].status == '2') {
							oType = '已经还款 ';
						} else {
							oType = '延期 ';
						}
						str3 += '<div  ><span>状态:</span><span>' + oType + '</span></div>';
						str3 += '<div  ><span>时间:</span><span class="color-red">' + data.cuishou_list[n].add_time + '</span></div>';
						str3 += '<div  ><span>备注:</span><span>' + data.cuishou_list[n].note + '</span></div>';
						str3 += '</div>';
					}
					document.getElementById('cuishou').innerHTML = str3;
					...
					$('#yqTotime').val(addDay);
					$('#yqGotime').val(showTime(parseInt($('#yqTotime').val()), $('#yqTime').val()));
					$('#yqMoney').val(data.jiekuan_list[listNum - 1].amount);
					if(data.type == 'A') {
						if($('#yqTotime').val() > 28) {
							$('#yqFee').val(Math.ceil($('#yqMoney').val() * 0.24 / 30 * 28));
						} else {
							$('#yqFee').val(Math.ceil($('#yqMoney').val() * 0.24 / 30 * $('#yqTotime').val()));
						}
					} else if(data.type == 'B') {
						if($('#yqTotime').val() > 14) {
							$('#yqFee').val(Math.ceil($('#yqMoney').val() * 0.24 / 15 * 14));
						} else {
							$('#yqFee').val(Math.ceil($('#yqMoney').val() * 0.24 / 30 * $('#yqTotime').val()));
						}
					} else if(data.type == 'C') {
						if($('#yqTotime').val() > 10) {
							$('#yqFee').val(Math.ceil($('#yqMoney').val() * 0.24 / 7.5 * 10));
						} else {
							//									debugger;
							$('#yqFee').val(Math.ceil($('#yqMoney').val() * 0.24 / 7.5 * $('#yqTotime').val()));
						}
					}
					/*延期费用*/
					$('#yqMoney').change(function() {
						if($('#yqTotime').val() == '') {
					...
					$('#yqTime').click(function() {
						laydate({
							...
							choose: function(datas) { //选择日期完毕的回调
								if($('#yqTotime').val() != '') {
									$('#yqGotime').val(showTime(parseInt($('#yqTotime').val()), datas));
								}
							}
						});
					});
					$('#yqTotime').change(function() {
						if($('#yqTime').val() != '') {
							$('#yqGotime').val(showTime(parseInt($('#yqTotime').val()), $('#yqTime').val()));
						}
						...
						if(r == true) {
							$.ajax({ //获取数据
								...
								data: {
									...
									beizhu: $('#bz').val(),
									delay_fee: $('#yqFee').val(),
									delay_days: $('#yqTotime').val(),
									jine: $('#yqMoney').val(),
									begin_date: $('#yqTime').val()
								},
								async: true,
								success: function(data) {
									if(data.status == '0') {
		...
		function newLoad() {
			...
			document.getElementById('shPeople').innerHTML = '催收员';
		}
