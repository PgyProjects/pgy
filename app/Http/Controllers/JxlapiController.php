...
class JxlapiController extends Controller
{
    ...
    private function _getUser()
    {
        ...
        return $jxl_user_data;
    }

    private function _getSessionId()
    {
        $openid = session('openid');
        if (!$openid) $openid = 'ohyMUwki96X68T3iJ7aHEHZenInU';
        return $openid;
    }
    ...
    public function jdqutoken()
    {
        ...
        $params = array(
            ...
            'basic_info' => array(
                "name" => $info['name'],
                "id_card_num" => $info['idCard'],
                "cell_phone_num" => $info['phone'],
            ),
        );
        $params = json_encode($params);
        //echo $params;
        ...
        if ($dd['code'] == 65557) {
            $urls = asset('jxl/jd1') . '?token=' . $dd['data']['token'] . '&shoujihao=' . $info['phone'] . '&website=' . $dd['data']['datasource']['website'];
    ...
    protected function failed($info, $code = 0)
    {
        $this->jsonReturn([
            'status' => $code,
            'message' => $info,
    ...
    /**
     * 获取运营商token
     * @auther Linzhonghuang
     */
    public function yysqutoken()
    {
        $jxl_user_data = DB::table('customers')->where('wx_openid', $this->_getSessionId())->first();
        $jxl_user_data = (array)$jxl_user_data;

        if (empty($jxl_user_data)) {
            header("Location: /home/bd1");
            die;
        }

        $forminfo = DB::table('custom_forms')->where('uid', $this->_getSessionId())->first();
        $forminfo = (array)$forminfo;
        if (!empty($forminfo) and isset($forminfo['auth_yys']) and 1 == $forminfo['auth_yys']) {
            die("
            <script>alert('运营商信息已经授权');</script>
        ...
        $name = $jxl_user_data['name'];
        $phone = $jxl_user_data['phone'];
        $idcard = $jxl_user_data['idCard'];
        ...
        if (!$dd['success']) die($dd['message']);
        $urls = asset('jxl/yys1') . "?token={$dd['data']['token']}&website={$dd['data']['datasource']['website']}";
        wang_tiaozhuan($urls);
    }
    ...
    public function yys1_ajax()
    {
        ...
        @$account = $_POST['shoujihao'];//手机号
        @$password = $_POST['password'];//手机密码
        ...
        if ($dd['success'] == true and !empty($dd['data']['process_code'])) {
            if (true == $dd['data']['finish']) {
                //修改数据库
                $token_name = DB::table('data_yys')->where('shoujihao', $account)->first();
                if (empty($token_name)) {
                    DB::table('data_yys')->insert(array('shoujihao' => $account, 'token' => $token));
                } else {
                    DB::table('data_yys')
                        ->where('shoujihao', $account)
                        ->update(array('token' => $token));
                }
                DB::table('custom_forms')
                    ->where('uid', $this->_getSessionId())
                    ->update(['auth_yys' => 1]);
                $this->success('提交成功');
            }elseif($dd['data']['type'] === 'ERROR'){
                $this->failed($dd['data']['content'], -1);
            } else {
                ...
                $this->failed('yys2?' . $query);
            }
        } else {
            $this->failed(empty($dd['message']) ? '查询失败' : $dd['message'], -1);
        }
...
//        }
    ...
    public function yys2_ajax()
    {
        ...
        if (!empty($dd['data']['process_code']) and $dd['data']['process_code'] == 10008) {

            //修改数据库
            $token_name = DB::table('data_yys')->where('shoujihao', $p_shoujihao)->first();
            if (empty($token_name)) {
                DB::table('data_yys')->insert(array('shoujihao' => $p_shoujihao, 'token' => $p_token));
            } else {
                DB::table('data_yys')
                    ->where('shoujihao', $p_shoujihao)
                    ->update(array('token' => $p_token));
            }
            DB::table('custom_forms')
                ->where('uid', $this->_getSessionId())
            ...
            $this->failed(empty($dd['message']) ? '短信验证失败' : $dd['message']);
        }
//        $params = json_encode($params);
//        //var_dump($params);
//        $content = http_post_json($url, $params);
...
//        if (@$dd['data']['process_code'] == 10008) {
//            $urls = site_url('jxl/yysok');
//            $arr = array('urls' => $urls, 'data' => array('process_code' => 10008), 'success' => true);
//            echo json_encode($arr, true);
//            //存入
//            $data = $this->db->get_where('jxl_user', array('shouji' => $p_shoujihao))->row_array();
//            $this->db->where('shoujihao', $p_shoujihao);
//            $this->db->update('jxl_data1', array('kongzhi_yys' => 1));
//            $this->db->insert('jxl_yys', array('token' => $p_token, 'shouji' => $p_shoujihao));
//            /*
//            if(empty($data)){
//                $in_data=array(
//                    'shouji'=>$p_shoujihao,
//                    'token'=>$p_token
//                    );
//                $this->db->insert('jxl_user',$in_data);
//            }*/
//
//        } else {
//            echo $content;
...
//            //$kongzhi_data=DB::table('custom_forms')->where('uid', $this->_getSessionId())->first();
...
//            echo $arr['zm_score'];//获得芝麻分
            ...
            if ($wxopenid = session('openid')) {
                if (false !== \CustomFormsModel::getInstance(1)->update([
                        'auth_zfb' => 1
                    ], [
                        'uid' => session('openid'),//uid 不是 wxopenid
                    ])
                ) {
//                    \CustomerModel::getInstance(1)->update([
//                        'input_zhima'   => $arr['zm_score'],
//                    ],[
//                        'wx_openid' => $wxopenid,
//                    ]);
                    \CustomerInfoModel::getInstance(1)->update([
                        'zhimafen' => $arr['zm_score'],
                    ], [
                        'wxid' => $wxopenid,
                    ]);
                    ...
                    //TODO:更新客户类型
                    $model = new customer();
                    $model->type($wxopenid, true);
//                    if (!$model->type($wxopenid, true)) {
//                        die('芝麻分暂时无法获取,请稍后重试，');
//                    }
                    //TODO:根据客户类型发送消息
                    $user_data = $this->_getUser();
                    $jump_url = 'http://test.pgyxwd.com/home';//消息跳转地址
                    if (!in_array($user_data['type'], ['A', 'B', 'C'])) {
                        //3小时后发送未通过消息
//                Timer::once("echo '$jump_url';",10);
                        Timer::once('
...
// 响应url
    ...
    $Muban->fucktheWeChatmuban(\'' . $wxopenid . '\', $template_id, $data, $url, $urls);
', 3 * 3600);
                    } else {
                        Timer::once('
    require_once __DIR__.\'/../../../Wang/Muban.php\';
...
// 响应url
    ...
    $Muban->fucktheWeChatmuban(\'' . $wxopenid . '\', $template_id, $data, $url, $urls);
', 20);
                        Utils::sendPassSMS($user_data['name'], $user_data['phone']);
                    }


                    ob_get_level() and ob_end_clean();
                    die('<script language="JavaScript">self.location="http://test.pgyxwd.com/home/index_2"; </script>');
                } else {
//            die(var_export([
//                $result,session('openid'),
//                \CustomFormsModel::getInstance(1)->getLastSql(),
//                \CustomFormsModel::getInstance(1)->getLastParams(),
//            ],true));
                    die('芝麻分暂时无法获取,请稍后重试。');
                }
            } else {
...
//            $url = str_replace('xinxi','test',$url);
    ...
    {
        $info = $this->_getUser();
        wang_sms_chushentongguo($info['name'], $info['phone']);
    }
