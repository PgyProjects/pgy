...
use Library\Traits\Singleton;
use PDO;
use PDOException;
use PDOStatement;
use Exception as Exception;
use Library\Dao\DaoAbstract;
use Library\Dao\MySQL;

/**
 * Class Dao
 * Database access object
 *
 * @method int lastInsertId(string $name = null)
 * @method string escape(string $fieldname) escape the keywork
 *
 ...
 *
 * @method array compile(array $components)
 *
 *
 * @package Sharin\Core
 */
class Dao
{
    ...
    protected static $name = 'dao';

    /**
     * @var DaoAbstract pdo driver instance
     */
    protected $pdo = null;
    /**
     * TODO:put fields info into cache if is development environment
     * @var array fields the table hold
     */
    ...
    public function __construct()
    {
        try {
            ...
            //log
            throw new DaoConnectException($e->getMessage());
        }
    }
        ...
        try {
            if (empty($inputs)) {
                ...
                if ($statement) {//query成功时返回PDOStatement对象
                    return $statement->fetchAll(PDO::FETCH_ASSOC);//成功返回
                } else {
    ...
    public function exec($sql, array $inputs = null)
    {
        ...
        try {
            if (!$inputs) {
                ...
                if (false === $statement) {
                    $this->error = self::fetchPdoError($statement);
                } else {
